'use strict';
/**
 * @description класс реальзующий модель таблицы
...
const Table = class Table {
  ...
  copyCell(cell, row) {
    if (cell instanceof Cell && row instanceof Row) {
      row.td.forEach((td) => td.id++);
      row.td = [new Cell(cell.value, row)].concat(row.td);
    }
  }
  addColumn(name) {
    this.th.push(name);
  ...
  destruct() {
    return this.tr.every((tr) =>tr.destruct()) ||
...
class Row {
  ...
  toString() {
    return this.td ? this.td.map((tr) => tr.toString())
        .join(this.__joinSimbol) : '';
  }
  destruct() {
    return this.td.every((td) => td.destruct()) ||
        Object.keys(this).forEach((key) => this[key] = null);
  }
}
class Cell {
